...
def preprocessing(df):
    ...
    categorical_cutting_features = ['신고인부호', '수입자부호', '해외거래처부호', '관세율_categorical']
    categorical_features = ['통관지세관부호', '신고인부호', '해외거래처부호', '특송업체부호', 
    ...
    for column in categorical_cutting_features:
        ...
        if column in ['신고인부호', '해외거래처부호', '관세율_categorical']:
            cnt = 30#10
        else:        #'수입자부호'
            cnt = 40#15
        filtered_count = filter(lambda x: x[1][0] >= cnt, submit_dict.items())
        # 비율이 아주낮거나 높으면 살림
        filtered_ratio = filter(lambda x: x[1][0] <= cnt and x[1][0] >= 6 and (x[1][1]/x[1][0] <= 0.15 or x[1][1]/x[1][0] >= 0.7)
